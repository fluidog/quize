...
static int access_mmap_cb(struct task_struct *task, struct vm_area_struct *mmap)
{
	unsigned int length;
	...
	return walk_process_vm(task, mmap, NULL, access_vm_cb);
}
...
int __init init_main(void)
{
	...
	walk_process_mmaps(pid, access_mmap_cb);
